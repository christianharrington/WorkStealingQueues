...
class QueueTests extends FlatSpec with Matchers with QueueHelper {
  ...
  "Taking from a WSQ" should "be LIFO" in runWithEveryQueue { q: TestQ =>
    val items = (1 to 100)
  ...
  "Stealing from a WSQ" should "be FIFO" in runWithQueues(ABPQueueImpl, ChaseLevQueueImpl, ChaseLevNaiveShrinkingQueueImpl) { q: TestQ =>
    val items = (1 to 100)
    items.foreach(i => q.push(i))
