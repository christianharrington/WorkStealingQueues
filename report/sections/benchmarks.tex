...
\todo{L\ae s}As with the Quick Sort case our implementation of XML Serialization is not idempotent. This is due to how a parent \texttt{Node} knows when all its children are done serializing. A simple atomic counter keeps track of how many children are done. This which means that if a child is processed more than once the counter in the parent \texttt{Node} does not accurately represent the number of completed children. As such the parent would have no way of knowing when it itself is done. 
...
An overview of our results with 16 threads can be seen in Figure~\ref{fig:all_results}. The result of our results can be found in Appendix~. \todo{Ref til rigtig appendix}
